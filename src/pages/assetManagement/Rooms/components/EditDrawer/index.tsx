...
const EditDrawer: React.FC<EditDrawerProps> = (props) => {
  ...
  const [columnList, setColumnList] =
    useState<{ id: string; name: string }[]>(columnData);
  const [columnItemRowVisible, setColumnItemRowVisible] =
  ...
  const setAddress = (addressData: any) => {
    ...
    form.setFieldsValue({
      address: addressStr,
      moreAddress: addressData.regeocode.formattedAddress,
    });
  };
  ...
  const columnListRemoveItem = (list: string[]) => {
    ...
    list.forEach((item) => {
      if (newColumnList.find((i) => i.id === item)) {
        newColumnList.splice(
          newColumnList.findIndex((i) => i.id === item),
          1,
        );
      }
    });
  ...
  const SortableContainers = SortableContainer(({ children }: any) => (
    <div>{children}</div>
  ));
          ...
          <Tabs.TabPane tab="基本信息" key="1" forceRender={true}>
            ...
            >
              <Input
                ...
                readOnly
                addonAfter={
                  <Button onClick={() => setAmapVisible((v) => !v)}>
                    <EnvironmentOutlined />
                  </Button>
                }
              />
            ...
            <Form.Item label={`机房照片(${pictureCount}/6)`}>
              <Upload
          ...
          <Tabs.TabPane tab="列信息" key="2" forceRender={true}>
                ...
                return (
                  <SortableItem index={i} key={i} className={styles.sortRow}>
                    <div className={styles.columnSortRow}>
                      <Checkbox
                        checked={
                          !!columnCheckedList.find(
                            (listItem) => listItem === d.id,
                          )
                        }
                        onChange={() => {
                          setColumnCheckedList(
                            columnCheckedList.find(
                              ...
                              ? columnCheckedList.filter(
                                  (item) => item !== d.id,
                                )
                              : [...columnCheckedList, d.id],
                          );
                        }}
                      />
                      <DragHandle />
                      {d.id === columnEditRow ? (
                        <>
                          ...
                          <CheckCircleFilled
                            ...
                            onClick={() => columnListChangeItem(d.id, newName)}
                          />
                        </>
                      ) : (
                        <div
                          style={{ minWidth: 500 }}
                          onDoubleClick={() => setColumnEditRow(d.id)}
                        >
                          {d.name}
                        </div>
                      )}
                      {d.id !== columnEditRow && (
                        <CloseCircleFilled
                          ...
                          onClick={() => columnListRemoveItem([d.id])}
                        />
                      )}
                    </div>
                  </SortableItem>
            ...
            {columnItemRowVisible && (
              <div className={styles.newItemRow}>
                <Input
                  ...
                  value={columnItemInputValue}
                  onChange={(e) => setColumnItemInputValue(e.target.value)}
                />
                <a className={styles.saveButton} onClick={columnListAddData}>
                  保存
                </a>
                <a
                  className={styles.cancelButton}
                  onClick={() => setColumnItemRowVisible(false)}
                >
                  取消
                </a>
              </div>
            ...
            <div className={styles.optionRow}>
              <a
                ...
                onClick={() => setColumnItemRowVisible(true)}
              >
                新建
              </a>
              <a
